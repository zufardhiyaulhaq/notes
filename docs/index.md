---
hide:
  - navigation
  - toc
---

# Zufar's Notes

Working notes I keep for myself — the quick things I look *up*, not read cover to
cover. The longer, polished writing lives on
[zufardhiyaulhaq.com](https://zufardhiyaulhaq.com).

Two kinds of notes:

<div class="grid cards" markdown>

-   :material-lightbulb-on: __[TIL](til/index.md)__

    One small thing I learned, dated and tagged. A running stream.

-   :material-book-open-variant: __[Guides](guides/index.md)__

    Full how-tos and runbooks, migrated from my notes and learning repos.

</div>

!!! tip
    Press `/` to search across everything — usually the fastest way to find a command.
